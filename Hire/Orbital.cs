                ...
                */
...
#if DEBUG
                Vector3d BOtime = rock.getPositionAtUT(time) - opos;
                ...
                if (rock.name == "Mun" || (hour == 0 && minute == 0))
                    Debug.Log(String.Format("time: {0} {1} ratio {2:f1}(>=1 for total eclipse) " +
                        "loffT{3:f1}k  loff{4}k  lim{5:f1}k " +
                        "BOt {6:f1}M BO {7:f1}M BOt-Or {8:f1}M BO-Or {9:f1}M "+
                        "Or_r {10:f1} De_r {11:f1}M R_r {12:f1}",
                        time_str, rock.name, ratio,

                        LOffTime.magnitude / 1000, LOff.magnitude / 1000, limbo / 1000,

                        BOmtime / 1000000, BOm / 1000000, (BOmtime - Or_r) / 1000000, (BOm - Or_r) / 1000000, Or_r, De_r/1000000, rr));
